...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `rehab_growth_tib <- read_csv("ais_rehab_growth_model.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `rehab_growth_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `rehab_growth_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_rehab_growth_model.csv")`
...
rehab_growth_tib$id <- factor(rehab_growth_tib$id)
rehab_growth_tib$intervention <- factor(rehab_growth_tib$intervention)
rehab_growth_tib$time <- factor(rehab_growth_tib$time)
```
...
```{r echo = TRUE, eval = FALSE}
rehab_growth_tib$intervention <- fct_relevel(rehab_growth_tib$intervention, "Wait list")
rehab_growth_tib$time <- fct_relevel(rehab_growth_tib$time, "t12", after = Inf)
```
...
```{r lin_plot-solution}
growth_plot <- ggplot(rehab_growth_tib, aes(time_num, resemblance, colour = intervention, fill = intervention))
growth_plot +
...
```{r err_bar-solution}
growth_mean_plot <- ggplot(rehab_growth_tib, aes(time, resemblance, colour = intervention))
growth_mean_plot +
...
```{r echo = TRUE, eval = FALSE}
rehab_ri <- lme(resemblance ~ time_num, random = ~ 1|id, data = rehab_growth_tib, method = "ML")
...
```{r lme_2-setup}
rehab_ri <- lme(resemblance ~ time_num, random = ~ 1|id, data = rehab_growth_tib, method = "ML")
rehab_rs <- lme(resemblance ~ time_num, random = ~  time_num|id, data = rehab_growth_tib, method = "ML")
```
...
rehab_ri <- lme(resemblance ~ time_num, random = ~ 1|id, data = rehab_growth_tib, method = "ML")
rehab_rs <- lme(resemblance ~ time_num, random = ~  time_num|id, data = rehab_growth_tib, method = "ML")
rehab_quad <- lme(resemblance ~ poly(time_num, 2), random = ~  time_num|id, data = rehab_growth_tib, method = "ML")
```
...
```{r sum_rehab-setup}
rehab_mod <- lme(resemblance ~ poly(time_num, 2) + intervention + poly(time_num, 2):intervention, random = ~  time_num|id, data = rehab_growth_tib, method = "ML")
```
...
rehab_lme <- lme(resemblance ~ time*intervention, random = ~ intervention|id, data = rehab_growth_tib)
anova(rehab_lme)
```
...
rehab_lme_ri <- lme(resemblance ~ time*intervention, random = ~ 1|id, data = rehab_growth_tib)
anova(rehab_lme_ri)
```
...
rehab_lme_cs<- lme(resemblance ~ time*intervention, random = ~ 1|id, data = rehab_growth_tib, correlation = corCompSymm(form = ~ intervention|id))
anova(rehab_lme_cs)
```
