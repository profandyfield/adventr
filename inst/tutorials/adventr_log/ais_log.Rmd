...
recovery_tib <- recovery_tib %>% 
  mutate(
    ...
    treatment = as_factor(treatment) %>% fct_relevel("No toggle switch"),
    recovered = as_factor(recovered) %>% fct_relevel("Not recovered")
    )
...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `recovery_tib <- read_csv("ais_rehab_log.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `recovery_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `recovery_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_rehab_log.csv")`
...
```{r echo = TRUE, eval = FALSE}
recovery_plot <- ggplot(recovery_tib, aes(dose, fill = recovered, colour = recovered))
recovery_plot +
...
`recovery_rob <- glmRob(recovered ~ treatment*dose, data = recovery_tib, family = binomial())`
