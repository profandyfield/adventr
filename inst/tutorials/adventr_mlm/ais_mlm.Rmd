...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `rehab_tib <- read_csv("ais_zombie_rehab.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `rehab_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `rehab_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_zombie_rehab.csv")`
...
This code recreates the `rehab_tib` tibble from itself then uses mutate to recreate the variables **intervention** and **gep_level**, and **id_clin** from themselves by placing each one in turn within the `as_factor()` function. I have explicitly declared that I want to use the `as_factor()` function from the package `forcats` because the package `haven` also has an `as_factor()` function.
...
```{r echo = T, eval = F}
rehab_alt_tib$id_clin <- fct_relevel(rehab_alt_tib$id_clin, "clinic_1", "clinic_2", "clinic_3", "clinic_4", "clinic_5", "clinic_6", "clinic_7", "clinic_8", "clinic_9", "clinic_10")
rehab_alt_tib$id_clin <- fct_relevel(rehab_alt_tib$id_clin, "clinic_10", after = 9)
rehab_alt_tib$id_clin <- fct_relevel(rehab_alt_tib$id_clin, "clinic_10", after = Inf)
```
...
```{r echo = T, eval = F}
rehab_alt_tib$intervention <- fct_relevel(rehab_alt_tib$intervention, "Wait list", "Gene therapy")
rehab_alt_tib$intervention <- fct_relevel(rehab_alt_tib$intervention, "Gene therapy", after = 1)
rehab_alt_tib$intervention <- fct_relevel(rehab_alt_tib$intervention, "Gene therapy", after = Inf)
rehab_alt_tib$intervention <- fct_relevel(rehab_alt_tib$intervention, "Wait list", after = 0)
```
...
rehab_alt_tib$id_clin <- fct_relevel(rehab_alt_tib$id_clin, "clinic_10", after = Inf)
rehab_alt_tib$intervention <- fct_relevel(rehab_alt_tib$intervention, "Gene therapy", after = 1)
rehab_alt_tib$gep_level <- fct_relevel(rehab_alt_tib$gep_level, "High intensity", after = 1)
...
`r hint_text("Tip: to group means by three variables you can use group_by(x, y, z). If you're doing this outside of the tutorial remember to load the packages tidyverse and gmodels")`
...
```{r echo = T, eval = F}
rehab_plot <- ggplot(rehab_tib, aes(intervention, resemblance, colour = id_clin))
rehab_plot +
...
`rehab_base <- gls(resemblance ~ 1, data = rehab_tib, method = "ML")`
...
`rehab_ri <- lme(resemblance ~ 1, random = ~ 1|id_clin, data = rehab_tib, method = "ML")`
...
`rehab_int <- lme(resemblance ~ intervention, random = ~ 1|id_clin, data = rehab_tib, method = "ML")`
...
`rehab_rs <- lme(resemblance ~ intervention, random = ~ intervention|id_clin, data = rehab_tib, method = "ML")`
...
```{r sum_tse-setup}
rehab_tse <- lme(resemblance ~ intervention + gep_level + tse_months, random = ~ intervention|id_clin, data = rehab_tib, method = "ML")
```
