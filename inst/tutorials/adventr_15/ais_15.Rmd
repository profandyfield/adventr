...
So we would use the same formula that we have used throughout, and specify our tibble as `mask_dat`. It's a good idea to save this model into a new object (lets call it `mask_bf`) because you can do useful things with it (we didn't do this for the `yuen()` functions, but you could).
