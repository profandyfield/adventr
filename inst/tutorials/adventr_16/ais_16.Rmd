...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `memory_tib <- read_csv("ais_16_memory_erasing.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `memory_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `memory_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_16_memory_erasing.csv")`
...
```{r scat-solution}
mem_plot <- ggplot(memory_tib, aes(group, recall))
mem_plot +
...
`r hint_text("Tip: R will order the groups alphabetically (in this case control, erase, replace). For the current data this is a sensible order because we expect recall to get progressively worse across those groups. However, if the alphabetical order of groups does not represent the order that you want, you can use the fct_relevel() function in the forcats package to re-order the levels of your grouping variable into the order you want.")`
...
robust(mem_mod, vcov.type = "HC4", conf.int = T)
```
...
mem_rob <- lmRob(recall ~ group, data = memory_tib)
summary(mem_rob)   
```
...
boot_result <- boot(boot_lm, formula = recall ~ group, data = memory_tib, R = 2000)
...
```{r mask_wrs-solution}
t1way(recall ~ group, data = memory_tib)
lincon(recall ~ group, data = memory_tib)
t1waybt(recall ~ group, data = memory_tib, nboot = 1000)
mcppb20(recall ~ group, data = memory_tib, nboot = 1000)
```
...
```{r echo = T, eval = F}
mem_bf <- anovaBF(formula = recall ~ group, data = data.frame(memory_tib))
mem_bf
```
