...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `implant_tib <- read_csv("ais_16_implanting.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `implant_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `implant_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_16_implanting.csv")`
...
`gather(implant_tib, key = "interference", value = "recall", -id)`
...
`implant_tidy <- gather(implant_tib, key = "interference", value = "recall", -id)`

or by using a pipe:

`implant_tidy <- implant_tib %>% gather(key = "interference", value = "recall", -id)`
