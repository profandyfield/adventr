...
`r hint_text("Tip: Near the top of my scripts I have a section marked *# load packages* fiollowed by a list of commands that load all of the packages I use in the script. (I also list them alphabetically because I'm anally retentive.) This makes it easy for me to see which packages the script relies on") `
...
`ha_tib <- read_csv("ais_ch03_ha.csv", header = FALSE)`
...
`ha_tib <- read_csv(file.path(Sys.getenv("HOME"), "Documents", "Data", "ais_ch03_ha.csv"))`
...
creates a tibble called `ha_tib` from whatever file you choose through the resulting dialogue box.
...
```
ha_tib <- ha_dat %>%
  mutate(mean_hum_kind = (humour + kind)/2)
```
...
```{r ha_4-solution}
ha_tib <- ha_tib %>% mutate(humour_sq = humour^2)
ha_tib
```
...
`ha_1618_tib <- ha_tib %>% filter(age >= 16)`
...
`ha_1618_tib <- ha_tib %>% filter(age != 15)`
...
`ha_15_tib <- ha_tib %>% filter(age == 15)`
...
```{r ha_5-solution}
kind_max_tib <- ha_tib %>% filter(kind == 10)
kind_max_tib
```
