...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `alice_tib <- read_csv("ais_17_alice_data.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `alice_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `alice_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_17_alice_data.csv")`
...
This code recreates the `alice_tib` tibble from itself then uses mutate to recreate the variables **picture** and **gene** from themselves by placing each one in turn within the `as_factor()` function. I have explicitly declared that I want to use the `as_factor()` function from the package `forcats` because the package `haven` also has an `as_factor()` function.
...
```{r plot-solution}
alice_plot<-ggplot(alice_tib, aes(gene, resemblance, colour = picture))
alice_plot +
...
robust(gen_mod, vcov.type = "HC4", conf.int = T)
```
...
gen_rob <- lmRob(resemblance ~ gene*picture, data = alice_tib)
summary(gen_rob)   
```
...
t2way(resemblance ~ gene*picture, data = alice_tib)
mcp2atm(resemblance ~ gene*picture, data = alice_tib)
pbad2way(resemblance ~ gene*picture, data = alice_tib, nboot = 1000)
mcp2a(resemblance ~ gene*picture, data = alice_tib, nboot = 1000)
```
...
```{r echo = T, eval = F}
gene_bf <- lmBF(formula = resemblance ~ gene, data = data.frame(alice_tib))
pic_bf <- lmBF(formula = resemblance ~ gene + picture, data = data.frame(alice_tib))
int_bf <- lmBF(formula = resemblance ~ gene + picture + gene:picture, data = data.frame(alice_tib))
```
...
robust(mem_cov_mod)
mem_cov_rob <- lmRob(recall ~ working_mem + group, data = mem_cov_tib)
summary(mem_cov_rob)

wm_bf <- lmBF(formula = recall ~ working_mem, data = data.frame(mem_cov_tib))
grp_bf <- lmBF(formula = recall ~ working_mem + group, data = data.frame(mem_cov_tib))
wm_bf
grp_bf/wm_bf
```
