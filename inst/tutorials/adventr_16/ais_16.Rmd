...
if (commandArgs()[1] == "/usr/lib/R/bin/exec/R" ){
    ...
    memory_dat$group <- forcats::as_factor(memory_dat$group) 
    } else {
...
Let's plot the data. Use the code box below to create an error bar chart with **group** on the *y*-axis and **recall** on the *x*-axis. If you feel like it, try to superimpose the raw data. (Hint, you can adapt the code that you used in the *adventr_15* tutorial.)
...
For now, the *Y* intercept ($b_0$) is 5, which is the value of recognition when when **group** is the reference category. As just described, **R** will have coded the variable **group** alphabetically (unless you tell it something different) so the reference category is the control group. So, ($b_0$) is 5 because the mean of the control group is 5. As we saw above, the two predictors are dummy variables, and their respective *b*s reflect the differences between means that the dummy variables code. **groupErase** will be the difference between the mean of the erase group (4.75) and the mean of the control group (5.00), which is $4.75 - 5.00 = -0.25$, and **groupReplace** will be the difference between the mean of the replace group (2.67) and the mean of the control group (5.00), which is $2.67 - 5.00 = -2.33$.
...
quiz(
  question("How would you interpret the information about the *F*-statistic?",
    ...
    answer("Overall recall scores can be significantly predicted from the group means. This implies that the group means are significantly different.", correct = T, message = "Because the *p*-value associated with *F* is 0.0967, which is more than the typical criterion of 0.05, we would conclude that using the group means does NOT help us to predict recall scores. In other words the group means are not significantly different."),
...
The `WRS2` package [@RN10205] wraps up a few of the many functions described by Wilcox to perform robust variants of tests [@RN5244]. We'll look at three functions that compare several means. They have a very similar format:
...
Like in previous tutorials (*adventr_11*, *adventr_14*, *adventr_14*) we can use the [BayesFactor package](http://bayesfactorpcl.r-forge.r-project.org/). In this scenario we use the `anovaBF()` function. This function has basically the same format as most of the other functions in this tutorial:
