...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `jig_tib <- read_csv("ais_c05_jigsaw.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `jig_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `jig_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_c05_jigsaw.csv")`
...
`boot(jig_tib$strength, statistic, R = 1000)`
...
`boot(jig_tib$strength, mean_boot, R = 1000)`
...
mean_boot <- function(data, index) mean(data[index], na.rm = TRUE)
boot.ci(boot(jig_tib$strength, mean_boot, R = 1000), type = "bca")
```
...
`boot.ci(boot(jig_tib$strength, mean_boot, R = 1000), type = "bca")$bca`
...
`boot.ci(boot(jig_tib$strength, mean_boot, R = 1000), type = "bca")$bca[4]`
`boot.ci(boot(jig_tib$strength, mean_boot, R = 1000), type = "bca")$bca[5]`
