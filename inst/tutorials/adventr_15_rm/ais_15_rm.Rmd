...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `calcite_tib <- read_csv("ais_15_calcite.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `calcite_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `calcite_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_15_calcite.csv")`
...
`gather(calcite_tib, key = "face_type", value = "recognition", -id)`
...
`calcite_tidy <- gather(calcite_tib, key = "face_type", value = "recognition", -id)`

or by using a pipe:

```
calcite_tidy <- calcite_tib %>% 
  gather(key = "face_type", value = "recognition", -id)
```
...
actual <- calcite_tidy %>%
  filter(face_type == "actual") %>%
  select(recognition)

different <- calcite_tidy %>%
  filter(face_type == "different") %>%
  select(recognition)
