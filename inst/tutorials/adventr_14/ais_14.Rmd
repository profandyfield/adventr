...
* Assuming that you save this file into the working directory, you can load it by executing:
    - `garlic_tib <- read_csv("ais_14_zombie_garlic.csv")`
...
* To load the file using a standard dialogue box to navigate your file system, execute:
    - `garlic_tib <- read_csv(file.choose())`
* You can load the file direct from the URL (i.e., you don't need to download it first) by executing:
    - `garlic_tib <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_14_zombie_garlic.csv")`
...
```{r scat-solution}
scat <- ggplot(garlic_tib, aes(garlic, latency))
scat +
...
* `subset`: this argument allows you to specify a condition by which you subset data (a bit like you might use in the `filter()` function). For example, if you want to include only zombies younger than 10 years you could set `subset = age < 10`. Of course, you can also achieve the same thing using a pipe (`garlic_tib %>% filter(age < 10) %>% lm(latency ~ garlic, data = .)`)
...
```{r echo = T, eval = F}
taser_rob <- lmRob(immobility ~ r_tms + taser, data = taser_tib)
summary(taser_rob)
```
...
robust(taser_mod, vcov.type = "HC4", conf.int = T)
```
...
```{r echo = T, eval = F}
boot_result <- boot(boot_lm, formula = immobility ~ r_tms + taser, data = taser_tib, R = 2000)
```
...
This creates an object called `bf_mod` based on the same type of formula that we put into the `lm()` function to specify the model (so this should be familiar). To set a prior include `rscaleCont = `. By default (i.e. if you exclude the option altogether) the prior is set at "medium" ($^\sqrt{2}/_4$), but you can specify `rscaleCont = "wide"` ($^1/_2$) or `rscaleCont = "ultrawide"` ($^\sqrt{2}/_2$). Finally, the data are specified. If you are using a tibble (rather than a data frame), we need to convert it to a data frame because (at the time of writing) the `BayesFactor` package doesn't consistently support tibbles. To do this conversion, we use the pipe to feed the tibble (`taser_tib`) into `data.frame()`, which converts it to a data frame, and then pipe this into the `regressionBF()` function. For our data (using the default prior) we could execute:

```{r echo = T, eval = F}
taser_bf <- taser_tib %>% 
  data.frame() %>% 
  regressionBF(immobility ~ r_tms + taser, data = .)
...
full_bf <- taser_tib %>% 
  data.frame() %>%
  lmBF(immobility ~ r_tms + taser, data = .)
```
