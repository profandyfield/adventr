...
bibliography: r_data_basics.bib
---
...
ha_dat <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_ch03_ha.csv")
...
This tutorial is one of a series that accompanies *An Adventure in Statistics* [@RN10163] and *Discovering Statistics Using R* [@RN4832] by me, [Andy Field](https://en.wikipedia.org/wiki/Andy_Field_(academic)). These tutorials contain abridged sections from the book so there are some copyright considerations but I offer them under a [Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License](http://creativecommons.org/licenses/by-nc-nd/4.0/), ^[Basically you can use it for teaching and non-profit activities but not meddle with it or claim it as your own work.]

* Who is the tutorial aimed at?
    - Primarily students enrolled on my *Linear models in statistics* module at the University of Sussex, but anyone teaching from or reading *An Adventure in Statistics* [@RN10163] or *Discovering Statistics Using R* [@RN4832] may find them useful.
* What is covered?
    - This tutorial looks at the basic concepts in **R**, such as installing packages, setting the working directory, getting help, getting data into **R**, creating variables, creating data frames and tibbles, reading in data files, manipulating variables within a tibble/data frame..
...
So, the next day, after catching up with Nick (who has no positive sightings of Alice from the memoryBank), Zach and The Head go to the Beimeni Centre. They are asked what they are doing there by a small distinguished-looking lady, Professor Catherine Pincus but when Zach says he is looking for Alice, she turns brusque and says ‘no comment’. As she turns to walk away, Zach grabs her by the arm and introduces himself as Alice’s boyfriend. Professor Pincus asks him to follow her. 

In her office, Zach outlines what has happened. When he mentions JIG:SAW, a word that had come up in the report that Alice has clearly meant him to read and that he has since discovered is an organization run by the WGA, Professor Pincus becomes flustered and offers to look at the information in Alice’s file that suggests she was unhappy in their relationship. After a good deal of statistical analysis, Professor Pincus concludes that Alice was very satisfied with her and Zach’s relationship. 

At that moment a cat walks in and greets Catherine and when Zach expresses confusion at this, she tells him he is being ridiculous, there is no cat. But there is a cat on her desk and she dismisses it as ‘the lab cat’ and puts Zach’s claim that the cat spoke down to the stress of missing Alice.  The cat waves and Zach thinks lifts its eyebrows at him as if to say something is going on. Professor Pincus goes back into teaching mode. It seems, on the evidence of Alice’s file and from looking at data about what women want generally, that Alice may not have left Zach. She dismisses Zach by saying she hopes Alice comes back but he is suspicious that she has been lecturing him to cover her discomfort at the mention of JIG:SAW. She tells him to use the data he has uncovered to find Alice, which Zach considers useless advice so he suggests he’d be better off finding answers at JIG:SAW. 

Professor Pincus warns Zach in no uncertain terms that there are some things that are best left unexplored and that unless he wants his life destroyed, to leave things well alone. The wall unfolds around her and she leaves.  

The cat steps forward, introduces himself as Milton – and offers to help Zach

## Getting started in **RStudio**

### The tidyverse

Superficially, the [tidyverse](http://www.tidyverse.org/) is a set of packages for **R** created by Hadley Wickham and his team at **RStudio** that are collated in a single package called *tidyverse*. However, it is much more than that. The packages within *tidyverse* (e.g., *ggplot2*, *dplyr*, *readr*, *tidyr*, and *tibble*) are built upon a common philosophy. This philosophy extend not just to how the packages and their functions work, but beyond that to a general philosophy of data science as outlined in Hadley's books [@RN11403; @RN11404], both of which are free on-line (see resources section). Throughout these tutorials we follow this *tidyverse* philosophy where possible. What this means in practical terms is this: in **R** there are always several ways to do the same thing, where one of those ways is the *tidyverse* way, that's the method we'll use.



### Keyboard shortcuts

In the previous tutorial we downloaded and setup **R** and **RStudio**. Open a new script file using the *File > New File > R Script* menu or by simultaneously pressing the **ctrl** + **shift** + **N** keys (**⌘** + **shift** + **N** on a mac).
...
`ha_1618_dat <- ha_dat %>% filter(ageYears >= 16)`
...
* `filter(ageYears >= 16)` used the filter function and sets a condition to select cases if the variable **ageYears** is greater than or equal (*>=*) to 16.

We could achieve the same thing in other ways. For example:

`ha_1618_dat <- ha_dat %>% filter(ageYears != 15)`

Does the same thing but sets the filter to be that the variable **ageYears** is not equal (*!=*) to 15, which will have the effect of excluding the 15 year olds.

If we wanted *only* the 15-year olds, we could do this by executing:

`ha_15_dat <- ha_dat %>% filter(ageYears == 15)`

to create a tibble called *ha_15_dat* that contains cases in *ha_dat* where the variable **ageYears** is equal to (*==*) 15.
