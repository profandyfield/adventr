...
if (commandArgs()[1] == "/usr/lib/R/bin/exec/R" ){
  mem_cov_dat <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_16_memory_cov.csv");
  mem_cov_dat$group <- forcats::as_factor(mem_cov_dat$group);
  alice_dat <- read_csv("http://www.discoveringstatistics.com/repository/ais_data/ais_17_alice_data.csv");
  alice_dat$picture <- forcats::as_factor(alice_dat$picture);
  alice_dat$gene <- forcats::as_factor(alice_dat$gene)
    } else {
...
* `effects` [@RN11414] to compute adjusted means
...
`r colFmt("Hint: to group means by more than one variable you can use group_by(x, y). If you're doing this outside of the tutorial remember to load the packages tidyverse and gmodels", cbGreen)`
...
`r colFmt("Warning: the following section is quite tricky", cbRed)`
...
Having done that we can follow the same process as the previous example for fitting the model (using `lm()`), getting *F*-statistics (using `Anova()`), fitting robust models (using `lmRob()` or `robust()`) and getting Bayes factors (using `lmBF()`). The only additional concern is getting the adjusted means, which we can do using the `effect()` function from the `effects` package:

 `effect("predictor", model)`
 
 So, you put in the name of your model and the name of the variable for which you want adjusted means. For the current example, we could, therefore, execute:
 
```{r echo = T, eval = F}
mem_cov_mod <- lm(recall ~ working_mem + group, data = mem_cov_dat)
effect("group", mem_cov_mod)
```

The first line creates the model predicting recall scores from working memory and experimental group. The second line returns the means for **group** adjusted for working memory (these are the means in each group at the average level of working memory).

###Fitting the model

Fitting the model is really the same as for the gene example. So, either in a separate **RStudio** session or in the box below generate a script to do the following:
...
* extract the adjusted means for **group**
