---
title: "adventr: hypothesis testing"
author: "Andy Field"
output:
  learnr::tutorial:
    progressive: false
...
teddy_sum <- teddy_tib %>% dplyr::group_by(study_n, group) %>% dplyr::summarize(
  ...
  sd = sd(self_esteem),
  ci_low = gmodels::ci(self_esteem)["CI lower"],
  ci_upp = gmodels::ci(self_esteem)["CI upper"]
)
...
* [ais_10_teddy_therapy.csv](http://www.discoveringstatistics.com/repository/ais_data/ais_10_teddy_therapy.csv).
...
* Assuming that you follow the workflow recommended in the tutorial **adventr_02** (see also this [online tutorial](http://milton-the-cat.rocks/learnr/r/r_getting_started/#section-working-in-rstudio)), you can load the data into an object called `teddy_tib` by executing:
    - `teddy_tib <- readr::read_csv("../data/ais_10_teddy_therapy.csv")`
...
* **id**: Participant ID
* **group**: whether the participant cuddled a teddy bear or the cardboard box that contained the teddy bear
* **study_n**: Factor that distinguishes the two studies in the data set. The first was based on a total *N* of 20 and the second was based on a total *N* of 200. We're going to use this variable to `dplyr::filter()` the tibble before we compute *d*.
* **self_esteem**: Self-reported self-esteem scores
