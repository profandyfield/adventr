...
teddy_sum <- teddy_tib %>% group_by(study_n, group) %>% summarize(
...
ted_plot <- ggplot(teddy_tib, aes(group, self_esteem))
ted_plot +
...
```{r task-solution}
zhang_plot <- ggplot(zhang_female_tib, aes(name, accuracy))
zhang_plot +
  stat_summary(fun.data = "mean_cl_normal", size = 1) +
...
cohen.d(formula = accuracy ~ name, data = zhang_female_tib, hedges.correction = T)
...
ttestBF(formula = accuracy ~ name, data = data.frame(zhang_female_tib))
```
