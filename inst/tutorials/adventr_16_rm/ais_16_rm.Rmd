...
The variables **id** and **interference** currently have the data type of character. This would be fine for a waht of what we want to do (because **R** is fairly intelligent about how it treats character variables). However, for some things (like assigning contrasts) we need these variables to be defined as factors. We can do this by using the `factor()` function, which we hae met before. For example, we could execute:
...
Note that levels have been assigned alphabetically. So, for **interference** the first level is *no_interference*, the second is *verbal* and the last *visual*. This is a sensible order for us, but if it wasn't we could use `fct_relevel()` to change the order.
